...
async def choix(ctx):
    ...
    embed = discord.Embed(
        title=data["question1"] ,
        colour=discord.Colour.blue()
    )

    fait_choix = await ctx.send(embed=embed, components=[action_row])
    ...
    if button_ctx.custom_id == "oui":
        await button_ctx.edit_origin(content=await choix2(ctx))
    else:
        await button_ctx.edit_origin(content="nop")



async def choix2(ctx):
    with open('file.json', 'r') as file:
        data = json.load(file)
...
bot.run("ODc1MzcxMDY0OTY2OTA1ODc3.YRUi1Q.ltdDFt59LBDyEamvFXBCn-WfbGI")